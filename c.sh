...
#export CACHE_SQLLINE_CLASSPATH=Y

false && ./sqlsh -o mysql '
select * from git_commits limit 1
'

./sqlsh -o mysql "
with committers (id, committer_date) as (values
...
select extract(year from ym) /* as Year */,
  ...
  d
from (
  select floor(c.commit_timestamp to month) as ym, count(*) as c /* , x.committer_date AS d */
  from git_commits AS c /*
  left join committer_emails AS e on c.author = e.email
...
./sqlsh -o mysql "
with committers (id, committer_date)
  as (select * from (values
    ...
    ('zhiwei',DATE '2016-01-01'),
    ('kgyrtkirk',DATE '2016-01-01'))),
  committer_emails (id, email)
    as (select id, id || '@apache.org' as email
      from committers)
select extract(year from ym) /* as Year */,
  extract(month from ym) /* as Month */,
  c
from (
  ...
  from git_commits AS c
  left join committer_emails AS e on regexp_replace(regexp_replace(c.author, 'Julian Hyde <', ''), '>', '') = e.email
  left join committers AS x on e.id = x.id
  group by floor(c.commit_timestamp to month)
  order by floor(c.commit_timestamp to month) desc limit 24)
"
