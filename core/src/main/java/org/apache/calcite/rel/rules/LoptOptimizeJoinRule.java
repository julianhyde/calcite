...
public class LoptOptimizeJoinRule
  ...
  private static RelNode createTopProject(
    ...
    for (int currFactor = 0; currFactor < nJoinFactors; currFactor++) {
      ...
      for (int fieldPos = 0;
          ...
          fieldPos++) {
        int newOffset = /*Y*/
            requireNonNull(factorToOffsetMap.get(currFactor), () -> "factorToOffsetMap.get(currFactor)") + fieldPos;
        if (leftFactor != null) {
          Integer leftOffset =
  ...
  private static @Nullable LoptJoinTree pushDownFactor(
    ...
    if (selfJoin) {
      ...
      Integer factor = /*Y*/
          requireNonNull(multiJoin.getOtherSelfJoinFactor(factorToAdd), () -> "multiJoin.getOtherSelfJoinFactor(" + factorToAdd + ") is null");
  ...
  private static @Nullable LoptJoinTree addToTop(
    ...
    if ((joinType == JoinRelType.LEFT) || (joinType == JoinRelType.RIGHT)) {
      condition = /*Y*/
          requireNonNull(multiJoin.getOuterJoinCond(factorToAdd), "multiJoin.getOuterJoinCond(factorToAdd)");
    } else {
      condition =
          addFilters(
              multiJoin,
  ...
  private static @Nullable LoptJoinTree createReplacementSemiJoin(
    ...
    int factIdx = /*Y*/
        requireNonNull(multiJoin.getJoinRemovalFactor(dimIdx), () -> "multiJoin.getJoinRemovalFactor(dimIdx) for " + dimIdx + ", " + multiJoin);
