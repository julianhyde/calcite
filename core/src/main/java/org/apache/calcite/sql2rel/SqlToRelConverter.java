...
import static org.apache.calcite.util.Pair.transform;
...
public class SqlToRelConverter {
  ...
  public @Nullable RelNode strcRoot;
  ...
  protected void convertSelectImpl(
      final Blackboard bb,
      SqlSelect select) {
    final int n = relBuilder.size();
    convertFrom(
        bb,
        select.getFrom());
    if (relBuilder.size() != n + 1) {
      for (int i = 0; i < relBuilder.size(); i++) {
        System.out.println("stack #" + i + ": " + relBuilder.peek(i));
      }
      throw new AssertionError("expected size " + (n + 1)
          + " but got size " + relBuilder.size());
    }
    ...
    if (RelOptUtil.isPureOrder(relBuilder.peek())
      ...
      if (!bb.top
        ...
        relBuilder.push(relBuilder.build().getInput(0));
        bb.setRoot(relBuilder.peek(), true);
      }
    }
    ...
    convertOrder(select, bb, collation, orderExprList, select.getOffset(),
        select.getFetch());

    if (select.hasHints()) {
      final List<RelHint> hints =
          SqlUtil.getRelHint(hintStrategies, select.getHints());
      relBuilder.push(attachHints(relBuilder.build(), hints));
    }
    bb.setRoot(relBuilder.peek(), true);
  }

  /** Attaches the given hints to the first {@link Hintable} node under
   * {@code r}. */
  private static RelNode attachHints(RelNode r, List<RelHint> hints) {
    return r.accept(
        new RelShuttleImpl() {
          boolean attached = false;

          @Override public RelNode visitChild(RelNode parent, int i,
              RelNode child) {
  ...
  private void distinctify(
    ...
    if (checkForDupExprs && (relBuilder.peek() instanceof LogicalProject)) {
      final LogicalProject project = (LogicalProject) relBuilder.peek();
      ...
      final List<RelDataTypeField> fields = project.getRowType().getFieldList();
      ...
      relBuilder.push(
          LogicalProject.create(relBuilder.build(), ImmutableList.of(),
              newProjects.leftList(), newProjects.rightList(),
              project.getVariablesSet()));
      bb.setRoot(relBuilder.peek());
      distinctify(bb, false);
  ...
  protected void convertOrder(
    ...
    relBuilder.push(
        LogicalSort.create(relBuilder.build(), collation,
    ...
    if (orderExprList.size() > 0 && !bb.top) {
      relBuilder.project(
          Util.skipLast(relBuilder.fields(), orderExprList.size()));
      bb.setRoot(relBuilder.peek(), false);
    }
  }
  ...
  private void convertWhere(
    ...
    final RexNode convertedWhere2 =
        RexUtil.removeNullabilityCast(typeFactory, convertedWhere); // TODO add to RelBuilder.filter

    // only allocate filter if the condition is not TRUE
    if (convertedWhere2.isAlwaysTrue()) { // TODO remove
      return;
    }

    boolean leaf = bb.inputs.contains(relBuilder.peek());
    relBuilder.transform(c -> c.withSimplify(false))
  ...
  private void replaceSubQueries(
    ...
    replaceSubQueries(bb, expr, logic, InputConsumer.ignore());
  }
  ...
  private void replaceSubQueries(
    ...
    replaceSubQueries(bb, expr, logic, null, inputConsumer);
  }
  ...
  private void replaceSubQueries(
    ...
    for (SubQuery node : bb.subQueryList) {
      substituteSubQuery(bb, node, inputConsumer);
    }
  }
  ...
  private void substituteSubQuery(Blackboard bb, SubQuery subQuery,
    ...
    case CURSOR:
      convertCursor(bb, subQuery, inputConsumer);
      return;
    ...
    case MULTISET_VALUE_CONSTRUCTOR:
      ...
      subQuery.expr =
          bb.register(relBuilder.peek(), JoinRelType.INNER, inputConsumer);
      inputConsumer.accept(relBuilder.build());
      return;
    ...
    case EXISTS:
      ...
      subQuery.expr = bb.register(converted.r, JoinRelType.LEFT, inputConsumer);
      return;
    case UNIQUE:
      return;
    case SCALAR_QUERY:
      ...
      subQuery.expr = bb.register(rel, JoinRelType.LEFT, inputConsumer);
      return;
...
//      relBuilder.push(converted.r); // TODO remove
      inputConsumer.accept(converted.r);
      return;
  ...
  private void substituteSubQueryOfSetSemanticsInputTable(Blackboard bb,
      SubQuery subQuery, InputConsumer inputConsumer) {
    final SqlBasicCall call = (SqlBasicCall) subQuery.node;
    ...
    subQuery.expr =
        bb.register(relBuilder.peek(), JoinRelType.LEFT, null, inputConsumer);
    inputConsumer.accept(relBuilder.build());
  }
      ...
      if (leftKeys.size() == 1) {
        ...
        rexComparison =
            RexUtil.composeConjunction(rexBuilder,
                transform(leftKeys, call.getOperandList(), (key, operand) ->
                    rexBuilder.makeCall(comparisonOp, key,
                        ensureSqlType(key.getType(),
                            bb.convertExpression(operand)))));
      }
  ...
  private RelNode convertQueryOrInList(Blackboard bb, SqlNode seek,
    ...
    if (seek instanceof SqlNodeList) {
      convertRowValues(bb, seek, (SqlNodeList) seek, false, targetRowType);
      return relBuilder.build();
    } else {
      RelRoot relRoot = convertQueryRecursive(seek, false, null);
      return relRoot.project();
    }
  }

  /** Converts values such as "1" or "(1, true)" or "(1, true), (2, false)"
   * into a {@link org.apache.calcite.rel.core.Values} relational operator.
   *
   * <p>Returns whether was successful. If successful, pushes the result onto
   * the {@link #relBuilder} stack.
   *
   * <p>If {@code allowLiteralsOnly} is false, it will never fail. */
  private boolean convertRowValues(
      Blackboard bb,
      SqlNode rowList,
    ...
    final List<ImmutableList<RexLiteral>> tupleList = new ArrayList<>();
    ...
    final int initialSize = relBuilder.size();
    for (SqlNode node : rows) {
      ...
      if (isRowConstructor(node)) {
        ...
        for (Ord<SqlNode> operand : Ord.zip(call.getOperandList())) {
          ...
          if ((rexLiteral == null) && allowLiteralsOnly) {
            // Restore stack.
            while (relBuilder.size() > initialSize) {
              relBuilder.build();
            }
            return false;
          }
      ...
      convertRowConstructor(bb, call);
    }
    if (relBuilder.size() == initialSize) {
      relBuilder.values(tupleList, rowType);
    } else {
      if (!tupleList.isEmpty()) {
        relBuilder.values(tupleList, rowType);
      }
      relBuilder.union(true, relBuilder.size() - initialSize);
    }
    leaves.put(relBuilder.peek(),
  ...
  protected void convertFrom(
    ...
    case EXCEPT:
    case UNION:
      RelRoot root = convertQueryRecursive(from, false, null);
      relBuilder.push(root.project());
      bb.setRoot(relBuilder.peek(), true);
      return;
  ...
  private void convertUnnest(Blackboard bb, SqlCall call,
    ...
    for (SqlNode node : call.getOperandList()) {
      replaceSubQueries(bb, node, RelOptUtil.Logic.TRUE_FALSE_UNKNOWN /* */,
          InputConsumer.of(rels) /* */);
    }

    final List<RexNode> exprs = new ArrayList<>();
    forEachIndexed(call.getOperandList(), (node, i) -> {
    ...
    if (rels.isEmpty()) {
      ...
      relBuilder.push(Iterables.getOnlyElement(rels));
    }
*/
    ...
    if (validator().config().conformance().allowAliasUnnestItems()) {
      ...
      relBuilder.uncollect(fieldNames, withOrdinality);
    } else {
      // REVIEW danny 2020-04-26: should we unify the normal field aliases and
      // the item aliases?
      relBuilder.uncollect(Collections.emptyList(), withOrdinality)
  ...
  private void convertIdentifier(Blackboard bb, SqlIdentifier id,
    ...
    final RelNode scan = toRel(table, hints);

    if (RelOptUtil.isPureOrder(scan)
        && removeSortInSubQuery(bb.top)) {
      relBuilder.push(scan.getInput(0));
    } else {
...
//    inputs.forEach(i -> relBuilder.build()); // TODO add list arg to replaceSubqueries
    ...
    relBuilder.push(
        LogicalTableFunctionScan.create(cluster,
            ...
            columnMappings));
    bb.setRoot(relBuilder.peek(), true);
    afterTableFunction(bb, call, (LogicalTableFunctionScan) relBuilder.peek());
  }
  ...
  private void convertJoin(Blackboard bb, SqlJoin join) {
    ...
    final RelNode leftRel = relBuilder.build();
    convertFrom(rightBlackboard, right);
    final RelNode tempRightRel = relBuilder.build();
    ...
    if (join.isNatural()) {
      ...
      case ON:
        Pair<RexNode, RelNode> conditionAndRightNode =
            convertOnCondition(fromBlackboard, join, leftRel, tempRightRel, InputConsumer.empty());
  ...
  private Pair<RexNode, RelNode> convertOnCondition(
      ...
      SqlJoin join,
      RelNode leftRel,
      RelNode rightRel, InputConsumer inputConsumer) {
    SqlNode condition =
    ...
    if (bb.rootIsNull() || bb.registered.isEmpty()) {
      newRightRel = rightRel;
    } else {
      newRightRel = bb.reRegister(rightRel);
  ...
  protected final void createAggImpl(
    ...
    replaceSubQueries(bb, groupList, RelOptUtil.Logic.TRUE_FALSE_UNKNOWN,
        SqlImplementor.Clause.GROUP_BY, InputConsumer.empty());
    ...
    try {
      ...
      bb.setRoot(relBuilder.peek(), false);
      bb.mapRootRelToFieldProjection.put(relBuilder.peek(),
          r.groupExprProjection);
      ...
      relBuilder.aggregate(
          relBuilder.groupKey(r.groupSet, r.groupSets.asList()),
      ...
      replaceSubQueries(bb, selectList, RelOptUtil.Logic.TRUE_FALSE_UNKNOWN,
          SqlImplementor.Clause.SELECT, InputConsumer.empty());
    ...
    Set<CorrelationId> variableSet = getCorrelationIds(bb, havingExpr);
  ...
  protected void gatherOrderExprs(
    ...
    checkArgument(!bb.rootIsNull(), "precondition: child != null");
    if (orderList == null) {
      return;
    }
  ...
  protected RelRoot convertQueryRecursive(SqlNode query, boolean top,
    ...
    case VALUES:
      ...
      return RelRoot.of(relBuilder.build(), kind);
    default:
  ...
  protected RelNode convertColumnList(final SqlInsert call, RelNode source) {
    ...
    final SqlNameMatcher nameMatcher = catalogReader.nameMatcher();
    Pair.forEach(targetColumnNames, columnExprs, (name, exp) -> {
      RelDataTypeField field = nameMatcher.field(targetRowType, name);
      assert field != null : "column " + name + " not found";
      sourceExps.set(field.getIndex(), exp);
    });
  ...
  /**
   * Converts a row constructor into a relational expression that returns a
   * single row; pushes that relational expression onto the {@link #relBuilder}
   * stack.
   *
   * @param bb             Blackboard
   * @param rowConstructor Row constructor expression
   */
  private void convertRowConstructor(Blackboard bb,
      SqlCall rowConstructor) {
    checkArgument(isRowConstructor(rowConstructor));
    convertMultisets(rowConstructor.getOperandList(), bb);
  }

  /** Converts a cursor, sending the result to a consumer. */
  private void convertCursor(Blackboard bb, SubQuery subQuery,
      InputConsumer inputConsumer) {
    ...
    final int iCursor = inputConsumer.size();
//    relBuilder.push(converted);
    inputConsumer.accept(converted);
    subQuery.expr = new RexInputRef(iCursor, converted.getRowType());
  }

  /** Converts multisets; pushes the result onto the {@link #relBuilder}
   * stack. */
  private void convertMultisets(final List<SqlNode> operands,
      Blackboard bb) {
    ...
    final List<SqlNode> lastList = new ArrayList<>();
    for (int i = 0; i < operands.size(); i++) {
      ...
      if (!lastList.isEmpty()) {
        joinList.add(ImmutableList.copyOf(lastList));
        lastList.clear();
      }
      relBuilder.push(
          Collect.create(input, call.getKind(),
  ...
  private void convertSelectList(
    ...
    final RelNode r0 = relBuilder.peek();
    relBuilder.projectNamed(exprs, fieldNames, true);

    final CorrelationUse p = getCorrelationUse(bb, relBuilder.peek());
    if (p != null) {
      relBuilder.build();
      ...
      relBuilder.push(r0)
          .projectNamed(project.getProjects(), fieldNames, true,
  ...
  private void convertQualify(Blackboard bb, @Nullable SqlNode qualify) {
    ...
    final LogicalProject projectionFromSelect =
        (LogicalProject) relBuilder.peek();
    ...
    try {
      ...
      relBuilder.build(); // discard the temporary root
      bb.setRoot(relBuilder.peek(), false);
    }
    ...
    if (qualifyWithReferencesRexNode.equals(qualifyRexNode)) {
      ...
      //    TableScan(tbl)
      //
      assert relBuilder.peek() == projectionFromSelect;
      relBuilder.build();
      relBuilder.push(projectionFromSelect.getInput())
          .project(
  ...
  /**
   * Converts a VALUES parse tree into a relational expression,
   * pushing the result onto the {@link #relBuilder} stack.
   */
  public void convertValues(SqlCall values,
      @Nullable RelDataType targetRowType) {
  ...
  private void convertValuesImpl(
    ...
    boolean success =
        convertRowValues(bb, values, values.getOperandList(), true,
            targetRowType);
    if (success) {
      bb.setRoot(relBuilder.peek(), true);
      return;
    ...
    for (SqlNode rowConstructor1 : values.getOperandList()) {
      ...
      forEachIndexed(rowConstructor.getOperandList(), (operand, i) ->
          exps.add(tmpBb.convertExpression(operand),
              SqlValidatorUtil.alias(operand, i)));
      if (tmpBb.rootIsNull()) {
        relBuilder.push(LogicalValues.createOneRow(cluster));
      }
      relBuilder.project(exps.leftList(), exps.rightList());
    }
  ...
  protected class Blackboard implements SqlRexContext, SqlVisitor<RexNode>,
    ...
    private final Map<CorrelationId, RexFieldAccess> mapCorrelateToRex =
        new HashMap<>();
    private final List<RegisterArgs> registered = new ArrayList<>();
...
//    private final List<RelNode> cursors = new ArrayList<>();
    ...
    public RelNode root() {
//      if (root != relBuilder.peek()) { // TODO
//        throw new AssertionError();
//      }
      return requireNonNull(root, "root");
    }

    public RelNode popRoot() {
      RelNode r = root();
      SqlToRelConverter.this.strcRoot = null;
      root = null;
      return r;
    }
...
//        throw new AssertionError();
    ...
    public RexNode register(
        ...
        JoinRelType joinType,
        InputConsumer inputConsumer) {
      return register(rel, joinType, null, inputConsumer);
    }
    ...
    RexNode register(RelNode rel, JoinRelType joinType,
        @Nullable List<RexNode> leftKeys, InputConsumer inputConsumer) {
      ...
      if (leftKeys != null) {
        ...
        relBuilder.push(newLeftInput);
        setRoot(relBuilder.peek(), leaves.remove(root()) != null);
...
//      if (root != relBuilder.peek()) { // TODO false &&
//        throw new AssertionError();
//      }
      SqlToRelConverter.this.strcRoot = root;
      this.root = root;
    }
...
//    }
  ...
  interface InputConsumer {
    void accept(RelNode r);
    default int size() {
      return 0;
    }
    default boolean shouldJoin() {
      return true;
    }

    static InputConsumer of(List<RelNode> list) {
      return new InputConsumer() {
        @Override public void accept(RelNode r) {
          list.add(r);
        }
        ...
        @Override public boolean shouldJoin() {
          return false;
        }
      };
    }

    static InputConsumer empty() {
      return r -> {
        throw new UnsupportedOperationException();
      };
    }

    static InputConsumer ignore() {
      return r -> {
      };
    }
  }
