...
public class JethroDataSqlDialect extends SqlDialect {
  ...
  @Override public boolean supportsFunction(SqlOperator operator,
    ...
    final Set<JethroSupportedFunction> currMethodSignatures =
    ...
    if (currMethodSignatures != null) {
      for (JethroSupportedFunction curr : currMethodSignatures) {
        if (curr.argumentsMatch(paramTypes)) {
          return true;
        }
      }
    }
  ...
  static class JethroSupportedFunction {
    ...
    boolean argumentsMatch(List<RelDataType> paramsList) {
      if (paramsList.size() != operandTypes.size()) {
        return false;
      }
      for (int i = 0; i < paramsList.size(); i++) {
        if (paramsList.get(i).getSqlTypeName() != operandTypes.get(i)) {
          return false;
        }
      }
      return true;
