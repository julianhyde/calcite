...
public class TemporalMaterializationWithJoinsTest extends SqlToRelTestBase {
  ...
  public static RelNode orderItems(RelBuilder builder){
    ...
    RelOptAbstractTable t = new RelOptAbstractTable(builder.getRelOptSchema(), "orders_items", vals) {};
    RelNode res = new TableScan(builder.getCluster(), builder.getCluster().traitSet(), Collections.emptyList(), t){};
    return res;
  }
  ...
  public static RelNode orders(RelBuilder builder){
    ...
    RelOptAbstractTable t = new RelOptAbstractTable(builder.getRelOptSchema(), "orders", vals) { };
    RelNode res = new TableScan(builder.getCluster(), builder.getCluster().traitSet(), Collections.emptyList(), t){};
    return res;
  }
  ...
  public static RelNode makeMaterialization(RelBuilder builder){
    ...
    //		  LogicalProject(order_items.order_id=[$1], $f3=[lookerFirst(template<order_items.amount >(), $2, $1, lookerFirst(null:NULL, $0, $1, $2, $3))])
    //		    LogicalTableScan(table=[[looker, order_items]])
    //			    "id" -> {BasicSqlType@28244} "DECIMAL(19, 0)"
    //				  "order_id" -> {BasicSqlType@28244} "DECIMAL(19, 0)"
    //				  "amount" -> {BasicSqlType@28244} "DECIMAL(19, 0)"
    ...
    return builder.aggregate(groupKey,
        ...
        builder.aggregateCall(SqlStdOperatorTable.SUM0, builder.field(1)).as("ORDER_ITEMS.ORDER_SUM")
    ).build();
  }
  ...
  public static RelNode makeQueryRel(RelBuilder builder){
    // (4)
    //		  LogicalProject($f0=[toDate_DATE_YEAR_asString(FLOOR($0, FLAG(YEAR)))], $f1=[$1])
    ...
    builder.clear();
    final RelNode rightBase = builder.push(orders).project(
        builder.alias(builder.field(0), "ID"),
        builder.alias(builder.field(4), "CREATED_AT")
    ).build();

    //(3) --
    int leftJoinIndex = leftBase.getRowType().getFieldNames().indexOf("ORDER_ID");
    int rightJoinIndex = rightBase.getRowType().getFieldNames().indexOf("ID");

    RexInputRef leftKeyInputRef = RexInputRef.of(leftJoinIndex,((LogicalProject) leftBase).getRowType());
    RexInputRef rightKeyInputRef =
        RexInputRef.of(leftBase.getRowType().getFieldCount()
            + rightJoinIndex,rightBase.getRowType());
    RexNode joinCond = builder.call(SqlStdOperatorTable.EQUALS, leftKeyInputRef, rightKeyInputRef);
  ...
  public static RelNode virtualMaterializationScanRelNode(RelBuilder builder, RelNode eqRel, String tableName){
    builder.clear();
    builder.push(eqRel);
    builder.as(tableName);
    final RelNode t = builder.build();
    return new TableScan(t.getCluster(), t.getTraitSet().getDefault(), Collections.emptyList(),
        new RelOptAbstractTable(builder.getRelOptSchema(), tableName, t.getRowType() ){}){};
  }

  public static String sql(RelNode rel){
    SqlPrettyWriter writer = new SqlPrettyWriter(SqlPrettyWriter.config().withClauseEndsLine(true)
