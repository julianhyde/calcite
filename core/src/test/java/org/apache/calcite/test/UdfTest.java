...
public class UdfTest {
  ...
  @Test public void testUserDefinedFunctionWithNull() {
    final String sql = "select\n"
        + "  \"adhoc\".my_det_plus(\"deptno\", 1 + null) as p\n"
        + "from \"adhoc\".EMPLOYEES\n"
        + "where 1 > 0 or nullif(null, 1) is null";
    final AtomicInteger c =
        Smalls.MyDeterministicPlusFunction.INSTANCE_COUNT.get();
    final int before = c.get();
    withUdf()
        .with(CalciteConnectionProperty.CONFORMANCE, SqlConformanceEnum.MYSQL_5)
        .query(sql)
