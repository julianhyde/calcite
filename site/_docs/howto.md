...
Create a class that extends `RelOptNewRule` (or occasionally a sub-class).
 ...
 */
class FilterFutzRule extends RelOptNewRule<FilterFutzRule.Config> {
  ...
  /** Rule configuration. */
  interface Config extends RelOptNewRule.Config {
...
`RelOptNewRule.Config` (or the config of a the rule's super-class).
