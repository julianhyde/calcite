...
public class JdbcMeta implements Meta {
  ...
  private final Map<Integer, Statement> statementMap = new HashMap<>();
   ...
   */
  protected static List<AvaticaParameter> parameters(ResultSetMetaData metaData)
      throws SQLException {
  ...
  protected static Signature signature(ResultSetMetaData metaData,
    ...
    return new Signature(columns(metaData), sql, parameters(metaData), null,
        CursorFactory.ARRAY);
  }
  ...
  public MetaResultSet prepareAndExecute(StatementHandle h, String sql,
    ...
    final Statement statement = statementMap.get(h.id);
    try {
      final ResultSet resultSet = statement.executeQuery(sql);
      return JdbcResultSet.create(resultSet);
    } catch (SQLException e) {
      throw propagate(e);
    }
  }
